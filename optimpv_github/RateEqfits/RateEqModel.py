...
def BT_model(parameters, t, Gpulse, t_span, N0=0, G_frac = 1,  equilibrate=True, eq_limit=1e-2, maxcount=1e3, solver_func = 'solve_ivp', **kwargs):
    ...
    if 'k_auger' in parameters.keys():
        k_auger = parameters['k_auger']
    else:
        k_auger = 0
    ...
    if equilibrate: # make sure the system is in equilibrium 
        ...
        while np.any(abs(RealChange) > eq_limit) and count < maxcount:
            if solver_func == 'odeint':
                r = odeint(dndt, rstart, t_span, args=(t_span, Gpulse, k_trap, k_direct, k_auger), tfirst=True, **kwargs)
                ...
                r = solve_ivp(partial(dndt,t_span = t_span, Gpulse = Gpulse, k_trap = k_trap, k_direct = k_direct, k_auger = k_auger), [t[0], t[-1]], [N0*G_frac], t_eval = t, method = method, rtol=rtol)
    ...
    if solver_func == 'odeint':
        r = odeint(dndt, rstart, t, args=(t, Gpulse_eq, k_trap, k_direct, k_auger), tfirst=True, **kwargs)
        return r[:,0], r[:,0]
    elif solver_func == 'solve_ivp':
        # r = solve_ivp(dndt, [t[0], t[-1]], rstart, t_eval = t, args=(t, Gpulse_eq, k_trap, k_direct), method = method, rtol=rtol)
        r = solve_ivp(partial(dndt,t_span = t, Gpulse = Gpulse_eq, k_trap = k_trap, k_direct = k_direct, k_auger = k_auger), [t[0], t[-1]], rend + N0*G_frac, t_eval = t, method = method, rtol=rtol)
